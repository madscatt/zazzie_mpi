...
int main(int argc, char **argv) {
    int **global, **local, **globaly, **localy, **globalz, **localz;
   ...
   if (size != xproc_frames) {
    ...
    MPI_Type_create_resized(type, 0, xn_frames/xproc_frames *sizeof(int), &subarrtype);
    ...
    MPI_Scatterv(globalptr, sendcounts, displs, subarrtype, &(local[0][0]),
                 xn_atoms*xn_frames/(xproc_frames), MPI_INT,
                 0, MPI_COMM_WORLD);
